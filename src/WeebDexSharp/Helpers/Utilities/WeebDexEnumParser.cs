...
/// <summary>
/// Utility for parsing enums with WD return results
/// </summary>
/// <typeparam name="T">The type of enum to parse</typeparam>
public class WeebDexEnumParser<T> : JsonConverter<T> where T : struct, IConvertible
{
	/// <summary>
	...
	{
		VerifyType(typeof(T));
		JsonSerializer.Serialize(writer, value.ToString().ToLower(), typeof(string), options);
	}
